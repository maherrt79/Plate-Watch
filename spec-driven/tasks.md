...
The project follows a "Local-First" development strategy, split into three major phases:

1.  **Phase 1: Local Development Foundation** (L1-L4)
    - Establish local Docker environment, local backend, and reactive web app.
    - Implement core "Edge Device" logic with local video simulation.
    - **Status**: Completed.

2.  **Phase 2: Advanced Features & Refinement** (A1-A4)
    - Implement advanced logic: Hotlists, MMC (Make/Model/Color), Direction, and Map Analytics.
...
- [x] **L4. Edge Device (Local Simulation)**
  ...
  - [x] L4.5 Input Configuration
    - Unified config for Mock/Video/Webcam modes with CLI overrides
...
- [ ] **A2. Advanced AI & Edge Intelligence**
  ...
  - [x] A2.2 Direction of Travel
    - [x] Edge: Probabilistic direction generation
    - [x] Backend/Frontend: Support for storing and displaying direction
  - [ ] A2.3 Dwell Time Calculation

- [x] **A3. Dashboard & Analytics**
  - [x] A3.1 Map Dashboard (Jableh)
      ...
      - [x] Frontend: Interactive Vehicle Filtering (Table <-> Map)
...
- [x] Phase 4: UI Overhaul (Modern Dark Theme, Glassmorphism, Premium UX)
...
- [ ] 2. Implement Cloud Backend database models and schemas  
 - [ ] 2.1 Create Sighting database model with proper schema  
   ...
   - *_Requirements: 8.4, 8.5, 9.3, 9.4, 9.5_*  
...
- [ ] 3. Implement Cloud Backend authentication and security with AWS services  
 - [ ] 3.1 Implement AWS Cognito JWT token verification for users  
   ...
   - Create FastAPI dependency get_current_user() that validates Cognito token  
 ...
 - [ ] 3.2 Implement API key authentication for edge devices  
   - Create FastAPI dependency get_api_key() that validates x-api-key header  
 ...
 - [ ] 3.3 Create device registration endpoint  
   ...
   - Generate API key using secrets.token_urlsafe()  
...
- [ ] 4. Implement Cloud Backend ingestion API endpoints  
 - [ ] 4.1 Create POST /api/v1/sightings endpoint for device data ingestion  
   ...
   - *_Requirements: 7.1, 7.4, 7.5, 7.6, 7.7, 9.1, 9.2, 9.7_*  
...
- [ ] 7. Set up Edge Device project structure and configuration  
 - [ ] 7.1 Initialize Edge Device Python project  
   - Create project directory structure: edge_device/  
 ...
 - [ ] 7.2 Implement configuration management  
   - Create config/device_config.yaml template with fields:  
     - locationId (string)  
     - apiEndpoint (URL)  
     ...
     - logDirectory (path)  
   - Create utils/config_loader.py to load and parse YAML configuration  
   - Implement configuration validation:  
     - Validate locationId matches pattern: ^[A-Za-z0-9_-]+$  
...
- [ ] 8. Implement Edge Device AI models and computer vision pipeline  
 ...
 - [ ] 8.2 Implement object tracking with SORT/DeepSORT  
   ...
   - Return tracked objects with tracker_id and bounding box  
...
- [ ] 9. Implement Edge Device video processing and sighting detection  
 ...
 - [ ] 9.2 Implement SightingManager for de-duplication  
   ...
   - Implement is_processed() method to check if tracker has been processed  
   - Implement mark_processed() method to mark tracker as processed  
   - Implement remove_tracker() method to clean up when vehicle exits frame  
...
- [ ] 10. Implement Edge Device local image storage  
 - [ ] 10.1 Create ImageStorage class for saving vehicle images  
   - Implement save_image() method to save frame to local filesystem  
...
- [ ] 11. Implement Edge Device cloud data transmission to AWS  
 - [ ] 11.1 Create CloudClient for AWS API Gateway communication  
   ...
   - Create send_sighting() method to POST JSON payload  
...
- [ ] 21. Configure and deploy Edge Device with AWS integration  
 ...
 - Set up log rotation  
 - *_Requirements: 1.1, 1.2, 5.5, 15.1_*
